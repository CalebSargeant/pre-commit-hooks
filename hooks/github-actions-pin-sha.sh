...
log()  { [ "${PIN_SHA_VERBOSE}" -ge 1 ] && printf '%s\n' "$*"; }
dbg()  { [ "${PIN_SHA_VERBOSE}" -ge 2 ] && printf '%s\n' "$*"; }
warn() { printf 'WARN: %s\n' "$*" >&2; }
...
yaml_glob='.github/workflows/**/*.{yml,yaml}'
...
resolve_sha() {
  ...
  if [ -z "$ref" ]; then
    sha=$(git ls-remote "$remote" HEAD | awk 'NR==1{print $1}')
    printf '%s' "${sha}"
    return 0
  fi

  # Branch
  sha=$(git ls-remote "$remote" "refs/heads/${ref}" | awk 'NR==1{print $1}') || true
  if [ -n "$sha" ]; then printf '%s' "$sha"; return 0; fi

  # Tag (prefer peeled for annotated tags)
  sha=$(git ls-remote "$remote" "refs/tags/${ref}^{}" | awk 'NR==1{print $1}') || true
  if [ -n "$sha" ]; then printf '%s' "$sha"; return 0; fi
  sha=$(git ls-remote "$remote" "refs/tags/${ref}" | awk 'NR==1{print $1}') || true
  if [ -n "$sha" ]; then printf '%s' "$sha"; return 0; fi

  # Fallback
  sha=$(git ls-remote "$remote" "$ref" | awk 'NR==1{print $1}') || true
  printf '%s' "${sha}"
}
